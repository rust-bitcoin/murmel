...
impl FilterCalculator {
            ...
            if self.timeout.lock().unwrap().is_busy_with(peer, ExpectedReply::Block) == false {
                ...
                if self.missing.is_empty() {
                    ...
                    if missing.last().is_some() && *missing.last().unwrap() == genesis.bitcoin_hash() {
                        ...
                        chaindb.add_calculated_filter(&genesis.bitcoin_hash(), &Sha256dHash::default(), &script_filter)?;
                        chaindb.add_calculated_filter(&genesis.bitcoin_hash(), &Sha256dHash::default(), &Self::tx_filter(&genesis)?)?;
            ...
            if let Some(header) = chaindb.get_header(&block_id) {
                ...
                if block.check_merkle_root() && block.check_witness_commitment() {
                    ...
                    if let Some(prev_script) = chaindb.get_block_filter(&block.header.prev_blockhash, 0) {
                        ...
                        chaindb.add_calculated_filter(&block_id, &prev_script.filter_id(), &script_filter)?;
                    }
                    if let Some(prev_tx) = chaindb.get_block_filter(&block.header.prev_blockhash, TXID_FILTER) {
                        chaindb.add_calculated_filter(&block_id, &prev_tx.filter_id(), &Self::tx_filter(block)?)?;
                    }
    ...
    fn tx_filter(block: &Block) -> Result<BlockFilter, Error> {
        ...
        writer.finish()?;
        Ok(BlockFilter::new(content.as_slice()))
    }
}